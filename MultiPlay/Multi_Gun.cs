...
public class Multi_Gun : MonoBehaviourPun,IPunObservable
{
    public enum State
    {
        Ready,
    ...
    private Multi_Shooter gunHolder;
    ...
    public void Setup(Multi_Shooter gunHolder)//총의 초기화 실행 총을 쥐고 있는 사람이 누구인지
    {
    ...
    {
        var damager = gunHolder.photonView.ViewID;
        photonView.RPC("shotProcessOnServer", RpcTarget.MasterClient, startPoint, direction, damager);
        magAmmo--;
        if (magAmmo <= 0) state = State.Empty;
    }

    [PunRPC]
    private void shotProcessOnServer(Vector3 startPoint, Vector3 direction, int damager)
    {
        ...
        if (Physics.Raycast(startPoint, direction, out hit, fireDistance, ~excludeTarget))
        {
            ...
            var target = hit.collider.GetComponent<Multi_IDamageable>();//대미지를 받을 수 있는 타입
            if (target != null)
            {
                Vector3 hitPoint = hit.point;
                Vector3 hitNormal = hit.normal;

                if (!target.checkDamager(gunHolder.gameObject)) return;
                target.ApplyDamage(damage, hitPoint, hitNormal, damager);//대미지를 입는 타입이라면 ApplyDamage안에 이펙트를 재생하는 기능을 가지고있음
            }
            else
