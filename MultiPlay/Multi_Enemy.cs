...
public class Multi_Enemy : Multi_LivingEntity
{
    ...
    public Multi_LivingEntity targetEntity;//적이 추적할 대상 유니티 인스펙터 창에서 보이지 않음
    ...
    public void Setup(float health, float damage,
        float speed, Multi_LivingEntity targetentitiy)//적이 생성될 때 적의 스펙 결정.
    {
        ...
        while (!dead)
        {
            ...
            else
            {
                ...
                foreach(var collider in colliders)
                {
                    ...
                    var livingEntity = collider.GetComponent<Multi_LivingEntity>();
    ...
    {
        if (!base.ApplyDamage(amount, hitPoint, hitNormal, damager)) return false;//공격을 받을 수 없는 상태
        
        if(targetEntity == null)//추적 대상을 못찾았는데 공격을 받음
        {
            Debug.Log(damager);
            targetEntity = PhotonView.Find(damager).GetComponent<Multi_LivingEntity>();
            Debug.Log(targetEntity.tag);
           // targetEntity = damageMessage.damager.GetComponent<Multi_LivingEntity>();
        }
        EffectManager.Instance.PlayHitEffect(hitPoint, hitNormal, transform,
